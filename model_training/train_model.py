...
# In your train.py, make sure the metrics structure matches what GitHub Actions expects:
...
if __name__ == "__main__":
    ...
    if results:
        ...
        best_model = min(results, key=lambda x: x.get('test_overall_rmse', float('inf')))
        best_rmse = best_model.get('test_overall_rmse', float('inf'))
        
        # Create the metrics structure expected by GitHub Actions
        metrics_for_github = {
            "best_model": {
                "name": best_model['model_name'],
                "rmse": float(best_rmse),  # Ensure it's a float, not inf
                "mae": float(best_model.get('test_overall_mae', 0.0)),
                "r2": float(best_model.get('test_overall_r2', 0.0)),
                "accuracy_10": float(best_model.get('test_overall_accuracy_10', 0.0)),
                "accuracy_15": float(best_model.get('test_overall_accuracy_15', 0.0)),
                "test_overall_rmse": float(best_rmse)  # Alternative key name
            },
            "all_models": results,
            "training_timestamp": str(datetime.now()),
            "total_models_trained": len(results),
            "rmse": float(best_rmse)  # Top-level RMSE for easy access
        }
        
        # Debug: Print what we're saving
        print(f"\n📊 METRICS STRUCTURE BEING SAVED:")
        print(f"   Best model: {best_model['model_name']}")
        print(f"   RMSE: {best_rmse}")
        print(f"   Type of RMSE: {type(best_rmse)}")
        
        # 🔧 Save results to the location expected by GitHub Actions
        with open("models/model_metrics.json", "w") as f:
            json.dump(metrics_for_github, f, indent=2)
        print("✅ Metrics saved to models/model_metrics.json")
        
        # Also save to root directory for backward compatibility
        with open("metrics.json", "w") as f:
            json.dump(results, f, indent=2)
        print("✅ Metrics also saved to metrics.json (backup)")
        
        # Print best model info
        print(f"\n🏆 BEST MODEL: {best_model['model_name']}")
        print(f"   RMSE: {best_rmse:.2f}")
        print(f"   R²: {best_model.get('test_overall_r2', 'N/A'):.3f}")
        
        # Check if best model meets GitHub Actions criteria
        if best_rmse < 40:
            print(f"✅ Best model meets RMSE criteria (<40) and will be uploaded to Hugging Face Hub!")
        else:
            print(f"⚠️  Best model RMSE ({best_rmse:.2f}) exceeds threshold (40) - will not be uploaded")
            
    else:
        print("❌ No models were successfully trained. Check the logs above.")
        # Create empty metrics file to prevent GitHub Actions errors
        import os
        os.makedirs("models", exist_ok=True)
        with open("models/model_metrics.json", "w") as f:
            json.dump({
                "error": "No models trained successfully", 
                "best_model": {"rmse": float('inf')},
                "rmse": float('inf')
            }, f, indent=2)